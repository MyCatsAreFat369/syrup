...
public class GuildSettingsManager {
    ...
    public GuildSettings getSettings(Guild guild) {
        try (var connection = databaseManager.getConnection()) {
            try (var statement = connection.prepareStatement("SELECT remove_old_roles, add_on_join, channel_id FROM GuildSettings WHERE guild_id = ?")) {
    ...
    public boolean setSettings(Guild guild, GuildSettings guildSettings) {
        try (var connection = databaseManager.getConnection()) {
            try (var statement = connection.prepareStatement("INSERT INTO GuildSettings (guild_id, remove_old_roles, add_on_join, channel_id) VALUES (?, ?, ?, ?) ON CONFLICT (guild_id) DO UPDATE SET remove_old_roles = EXCLUDED.remove_old_roles, add_on_join = EXCLUDED.add_on_join, channel_id = EXCLUDED.channel_id")) {
                statement.setLong(1, guild.getIdLong());
                statement.setBoolean(2, guildSettings.removeOldRoles());
                statement.setBoolean(3, guildSettings.addOnRejoin());
                statement.setLong(4, guildSettings.channelID());
